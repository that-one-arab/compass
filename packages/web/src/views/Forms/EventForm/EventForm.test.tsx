...
test("should call onConvert when meta+shift+, keyboard shortcut is used", async () => {
  ...
  await act(async () => {
    // Simulate pressing Meta, then shift, then ','(which is the '<' key, because
    // pressing shift and comma key produces '<'), then releasing them
    await userEvent.keyboard("{Meta>}{shift>}{<}{/Meta}{/shift}");
  });
